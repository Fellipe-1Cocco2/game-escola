...
const Pergunta = require('../models/Pergunta'); // Importa o novo modelo
...
const criarSala = async (req, res) => {
    try {
        ...
        if (salaExistente) {
            return res.status(409).json({ message: 'Uma sala com este nome já existe.' });
        }
...
const excluirSala = async (req, res) => {
    try {
        ...
        if (!sala.criador.equals(req.professor._id)) {
            return res.status(403).json({ message: 'Apenas o criador da sala pode excluí-la.' });
        }
...
const adicionarPerguntaDoBanco = async (req, res) => {
    try {
        ...
        if(perguntaJaExiste) {
            return res.status(409).json({ message: 'Esta pergunta já foi adicionada a esta tarefa.' });
        }
...
// ... (Restante do código existente, como criarSala, excluirAluno, etc.)
module.exports = {
